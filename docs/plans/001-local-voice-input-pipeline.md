...
### Phase 8: 実用化・チューニング
- [x] エラー文言・通知（UserNotifications で fallback/失敗通知, 設定で ON/OFF）
- [x] ログイン項目（SMAppService で launchAtLogin 連動）、Last Result コピー、モデル可用性表示
- [x] 設定整理（並列度に再起動注記）、アイコン状態、ログ整理
- [ ] 並列度 2 ノブの実測ベンチ → 既定値確定（**実機動作確認が必要なため未実施**。UI で調整可能）
- [x] 完了条件: 実用化要素を実装しビルド成功。日常利用の安定性は実機要動作確認

---

## 実装ステータス（2026-06-09 時点）

全 8 フェーズを実装し、各フェーズで xcode MCP ビルド成功・警告ゼロを確認済み。
その後、開発者の実機フィードバックを反映して UI・整形・言語管理・多言語対応を大きく改修した（下記「実装後の改修」）。
**現状のアプリの全体像は [`docs/app_overview.md`](../app_overview.md) を参照**（本プランは設計意図・パイプライン詳細の一次資料として残す）。
音声・モデル・挿入・権限まわりの一部は引き続き実機動作確認が必要（「実機動作確認 TODO」参照）。

### 実装後の改修（実機フィードバック反映, 2026-06-09）

実機検証を通じて、本プラン記載から以下を変更した。設計の核（3 順序レイヤ + 2 並列ノブのパイプライン）は不変。

- **メニューバーを AppKit 化**: `MenuBarExtra` はラベルが ObservableObject 変化を再描画しづらくアイコンアニメが効かないため、`NSStatusItem`（`UI/MenuBar/StatusItemController.swift`）で直接管理し Timer で明滅。状態: 待機=`mic` / 録音中=`mic.fill` 明滅 / 処理中=`ellipsis.bubble` 明滅。
- **設定ウィンドウを自前 NSWindow 化**: macOS では AppKit から SwiftUI Settings シーンを開けない（"Please use SettingsLink"）ため `SettingsWindowController` で自前管理。`orderFrontRegardless` + `moveToActiveSpace` で確実に最前面化。
- **設定タブを 2 つに集約**: 「一般」（権限/起動/ホットキー/整形/カスタム指示を集約）と「言語」のみ。Diagnostics タブは廃止。
- **権限 UI を要許可時のみ表示**: 不足がある時だけ「一般」タブ最上部に目立つ形で不足分を表示し、全許可なら非表示。
- **ユーザー設定から外した項目（内部定数化）**: 最大録音秒数（内部 300s 安全上限）、`outputSafetyFactor`（内部 1.15）、並列度 2 ノブ（UI なし・既定 1/2 で起動時読込）。
- **挿入方式を auto 固定**: ユーザー選択を廃止（常に AX 直接挿入 → 失敗時クリップボード）。`InsertionMode` 列挙は内部的には残置。
- **通知を廃止しアラート化**: `UserNotifications` をやめ、**挿入に完全失敗した時のみ `NSAlert`** で通知（クリップボード退避を案内）。クリップボード経由で挿入成功した場合は無通知。
- **整形プロンプト改善**: 「transcript editor」として枠付けし、入力への回答・翻訳・要約を強く禁止。文末に疑問符を使う言語のみ「疑問文なら ? を付す」ルールを追加。
- **カスタム整形指示（新機能）**: 整形モード下に最大 200 字の自由記述欄。安全ルールを保ったままプロンプトへ注入（翻訳・回答・要約はしない）。チャンクのトークン計算にも反映。
- **整形モードに説明 + 例を表示**。
- **言語管理の強化**: DL 済みに「削除」（`AssetInventory.release`）ボタン。システム言語は OS 管理で消せないため削除ボタン非表示。選択中の言語を削除したらシステム言語へ自動復帰。デフォルト言語をリスト先頭に並べ替え。
- **ホットキーを自由入力化**: 任意のキーを 1 つ押して登録するレコーダー方式（修飾キーは flagsChanged で捕捉）。
- **多言語対応（i18n）**: String Catalog（`vkey/Localizable.xcstrings`）で UI を多言語化。**開発言語=英語、日本語訳を収録**。SwiftUI は `Text` リテラル、AppKit/モデル計算プロパティは `String(localized:)`。`knownRegions` に `ja` を追加。

### 実機動作確認 TODO（ビルドのみ検証済み・要動作確認）
- [ ] 権限フロー: 初回起動でマイク/音声認識/アクセシビリティ/入力監視を付与 → 各機能が動く
- [ ] ホットキー: 既定 Right Command の押下中録音・解放で確定（修飾キーの flagsChanged 検出）
- [ ] 録音→文字起こし: 短文の日本語/英語が正しくテキスト化される
- [ ] 言語資産 DL: 未DL言語を選ぶと AssetInventory でダウンロードされる
- [ ] 整形: フィラー除去・句読点補完が効き、翻訳・要約・改変が起きない
- [ ] トークン分割: 長文(3000〜5000字)で exceededContextWindowSize を出さず分割整形
- [ ] 順序保証: 連続発話 A→B→C が FIFO×チャンク順で挿入される
- [ ] 挿入: TextEdit/Notes/Safari/Slack で AX 直接挿入、失敗時クリップボード fallback
- [ ] 並列度ベンチ: `maxConcurrentModelCalls` を 1↔3 で実測し既定値を確定
- [ ] 通知・ログイン項目・Last Result コピーの動作
